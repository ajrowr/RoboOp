...
RoboOp currently supports these callback types:

### `response_complete`

This callback is triggered when the model has finished generating a complete response (including for streaming responses - but not for canned responses, since those are not generated by the model). The `data_tuple` contains an `anthropic.types.message.Message` object passed through from the Anthropic API. Note that this callback only fires after tool use is resolved, which can entail multiple turns; in other words, it only fires when the model is ready for the user's next prompt.
...
Fires when the model has provided a response for a single conversation turn, but before tool calls are executed. So if complex tool use is occurring then it may fire multiple times consecutively. The `data_tuple` contains `msg_text` (the extracted text of the response - NOT a full Anthropic message object, as these are not necessarily available until after the entire response is complete).
...
class CodeBot(Bot):
    sysprompt_text = """You are an autonomous coding assistant. You can create files, 
    run bash commands, and iterate on tasks until complete. When given a task, break it 
    down and use tools to accomplish it. Continue until the task is fully complete."""
    ...
    class WriteFile(Tool):
        ...
        def __call__(self, filepath: str, content: str):
            print(f"Writing {len(content)} bytes to file:", filepath)
    ...
    class RunBash(Tool):
        ...
        parameter_descriptions = {
            ...
            'show_output': 'Whether or not to display the output on the terminal'
        }
        def __call__(self, command: str, show_output:bool):
...
>>> def turn_complete_callback(conversation, data_tuple):
...     msg, = data_tuple
...
Writing 206 bytes to file: Umbrella/__main__.py
Writing 4703 bytes to file: Umbrella/colorful_hello.py
